...
def numbers_not_divisible_by_2_or_5(start_from=1):
    n = 1
    while True:
        if n % 2 != 0 and n % 5 != 0:
            yield n
...
def d(n, x, phi):
    ...
    for i in range(7):
        dig = (n - a - 1 - i) % b + a + 1
        SHADOW[(n, 10 ** i * x)] = extract(10 ** i * x, dig)
    return SHADOW[(n, x)]
...
def solve(n, lim=None):
    ...
    #gen = numbers_not_divisible_by_2_or_5(start_from=2)
